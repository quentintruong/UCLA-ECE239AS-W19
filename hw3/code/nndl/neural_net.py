...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    # ================================================================ #
    h1 = np.maximum([0], np.matmul(X, W1.T) + b1)
    scores = (np.matmul(h1, W2.T) + b2)
    # ================================================================ #
    # END YOUR CODE HERE
    # ================================================================ #
    ...
    regularization = 0.5 * reg * (np.sum(np.square(W1)) + np.sum(np.square(W2)))
    softmax = np.sum(np.log(np.sum(np.exp(scores), axis=1)) - scores[np.arange(scores.shape[0]), y]) / X.shape[0]
    loss = softmax + regularization
    # ================================================================ #
    # END YOUR CODE HERE
    # ================================================================ #
    pass
    ...
    probabilities = np.exp(scores) / np.sum(np.exp(scores), axis=1).reshape(X.shape[0], 1)
    probabilities[np.arange(N), y] -= 1
    dz = np.matmul(probabilities, W2).T * (np.matmul(X, W1.T) + b1 > 0).T / N
    
    grads['W2'] = (np.matmul(probabilities.T, h1) / N) + (reg * W2)
    grads['b2'] = np.sum(probabilities, axis=0) / N
    
    grads['W1'] = np.matmul(dz, X) + (reg * W1)
    grads['b1'] = np.sum(dz, axis=1)
    # ================================================================ #
    # END YOUR CODE HERE
    # ================================================================ #
  ...
  def predict(self, X):
    ...
    # ================================================================ #
    h1 = np.maximum([0], np.matmul(X, self.params['W1'].T) + self.params['b1'])
    scores = (np.matmul(h1, self.params['W2'].T) + self.params['b2'])
